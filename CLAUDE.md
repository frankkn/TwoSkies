...
- 預報常駐顯示在每片天空下方（不需點擊）：未來 24 小時逐時（時間/圖示/溫度/
  降雨%——0% 也顯示；畫面固定 6 格、其餘橫向捲動，右緣漸淡提示可捲）＋
  未來 7 天；只有溫度和雨，時間用該地的時鐘
- 七天列可視高度約 4 行，其餘垂直捲動（底緣漸淡提示）；每行「星期・圖示・
  雨%・低—高溫」緊鄰排列，不把溫度推到右邊、行內不留大空白。
  點某一天就地展開（手風琴）該日完整 24 小時的逐時列（格式同上方），
  再點收回；資料點擊時現抓、當次快取
- 捲動的手感：手機用手指滑，桌機滑鼠按住拖曳（grab 游標），
  拖曳過程不誤觸點擊
- 整個預報區塊限寬（約 400px）靠左——寬螢幕不撐滿，右半邊留給天空。
  天氣圖示是彩色的（晴日太陽、晴夜月亮、雲、雨、雪），iPhone 天氣的視覺語彙
- 打卡是一行半透明文字「我來看過你的天空了」（不做成按鈕的樣子——它是天空的
  一部分），當天點過即安靜地變為「今天來看過了」（不彈窗、不慶祝）
...
- 等待配對中（邀請已建立、對方未兌換）：天空上低調顯示邀請碼
  （純文字的複製連結/取消邀請）——這是短暫的過渡，使用者正需要這串碼
