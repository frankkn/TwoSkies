...
- **單一畫面，上下兩片天空**：上半是對方的（依天氣代碼與對方當地晝夜渲染：晴/雨/雪/雲、
  ...
  這個畫面本身就是產品
- **兩種檢視模式**（配對後，設定裡切換；裝置偏好存 localStorage `twoskies.viewMode`，
  不同步雲端）：預設「兩片同框」＝上述畫面；「一片大天空」（focus 模式）＝對方天空
  全螢幕、自己的天空縮成底部一道**地平線條**（HorizonStrip：迷你天空為底、稱呼·城市、
  溫度、visitedBy 短句），點整條互換焦點——換成自己全螢幕時對方的條移到頂部
  （空間語言不變：對方在上、自己在下）。每次進入 focus 模式 focused 重設為對方；
  APK 從背景喚回不是 remount，focused 不重設（誠實記錄）。SkyPane 互換必須帶
  `key={uid}`——ForecastBlock 的手風琴快取只以日期為 key，實例重用會把兩地資料串戲
- **打卡按鈕在對方那片天空的 footer**（兩種模式皆然）——「我來看過你的天空了」
  發生在看對方天空時，語意歸位。focus 模式看自己的天空時**刻意沒有打卡入口**
  （點條回到對方的天空再打卡），這是設計不是缺口；visitedBy 標記始終跟著自己的
  天空（同框/全螢幕＝footer 完整句，地平線條＝短句「{稱呼}來看過你」）
- 每片天空顯示：城市名、目前溫度（大字，下方兩行小字：最高/最低溫、
  天況「晴天/多雲/有雨/降雪」）、天氣動畫；對方那片多顯示當地時間
- 預報常駐顯示在每片天空下方（不需點擊）：未來 24 小時逐時（時間/圖示/溫度/
  降雨%——0% 也顯示）＋未來 10 天（Open-Meteo forecast_days=10，iPhone 同規格）；
  只有溫度和雨，時間用該地的時鐘。
  逐時格數依可用寬度動態分配（每格約 64px，視窗越寬一眼看到越多），
  其餘橫向捲動，右緣漸淡提示可捲。
  **預報密度兩檔**（ForecastBlock 的 density）：全螢幕單片天空（focus/solo/pending）
  ＝cozy——iPhone 式寬鬆，字級行距放大、逐日每行 grow 平分剩餘高度、貼近滿版；
  兩片同框＝compact——緊湊小字、卡片貼合內容、留白讓給卡片下方的天空呼吸
- **天氣叮嚀**（src/weather/nudge.ts，預報卡頂部、逐時列上方，iPhone 同位）：
  由確定性規則從 bundle 推導（Open-Meteo 無文字摘要），cozy 顯示至多兩句（。連接）、
  compact 只顯示第一句。規則庫：降雨開始（未來 12h 機率首達 60%，跨午夜自動冠「明天」）、
  降雨結束（連兩小時 <30%，**且模型先真的說過有雨**——證據守門，否則毛毛雨＋無資料會
  捏造出每小時往後滑的假停雨時間）、明天走勢（高溫差 ≥4°，可接「且有雨」）、
  陣風（未來 12h ≥50 km/h——40 在季風季是壁紙）。措辭鐵律：永遠帶「預計/大約」的
  誠實 hedge、只描述不建議（「注意/小心」製造焦慮）；沒有值得說的就整行不渲染；
  **刻意不做 hysteresis**——純函數無狀態就是誠實，句子隨預報進出是預報本身在變。
  規則引擎不讀 client 時鐘（時間只來自 bundle 的城市當地 hour）、不 import Firebase。
  單元測試在 tests/nudge.test.ts（`npm run test:unit`；裸 `vitest run` 會連 rules
  測試一起跑、沒 emulator 會炸）
...
- 設定入口是稱呼右側一顆低調的齒輪：稱呼/城市、檢視模式、配對功能（邀請/解除）、
  登出都在裡面；位置改動後兩邊天氣立即重抓。齒輪的落點——兩片同框＝上片（對方）
  名字旁、focus 模式＝跟著當前全螢幕那片（離開 focus 的唯一路，兩種焦點都要搆得到）、
  solo/pending＝自己名字旁
